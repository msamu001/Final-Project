...
\begin{itemize}
\item The size of a graph is the number of edges contained inside the graph.
\item The order of a graph is the number of vertices inside the graph.
\item The degree of a vertex is the number of edges connected to the vertex.
\end{itemize}
...
\item	Vertices can exist without any connecting edges
\item	Vertices can exist with connecting edges
\item	raversal between all connected vertices is possible
\item	Edges cannot exist without connected vertices
\item	The graph can be deconstructed
\end{itemize}
I implemented all the Unit tests for the EWG with the exception of Traversal and Deconstruction which I will complete later today. I have filled out the EWG skeleton class’ methods and have ensured that it passes all tests that have been implemented.
I have begun researching how to create a Gantt chart and designed SDS for MST.\\

Weekly Report

Supervisor Notes
Unfortunately I have been unable to meet my supervisor due to the strikes. I have added new objectives and removed complete tasks from the last week
This week’s checklist should cover the following 2 weeks
\begin{itemize}
\item	Designed SDS for MST
\item	Create Requirements for SDS
\item Implement Unit tests for SDS
\item	Implement SDS to pass tests
\item	Create Requirements for Agent class
\item	Implement Unit tests for Agent class
\item	Pass all Unit tests for Agent class
\item	Create Gantt Chart
\end{itemize}
This week’s progress
I have created the list of requirements for SDS for each phase and split SDS into two main classes Agent and SDS.
Requirements for SDS Initialization phase:
\begin{itemize}
\item	Agents are generated during the initialization phase
\item	Every agent has a hypothesis after the initialization phase
Requirements for SDS Test phase:
\end{itemize}
\begin{itemize}
\item	Every Agent’s fitness is calculated
\item	The total fitness of all agents is equal to 100 
\item	Agents within the activation threshold become active
\item	Agents outside of the activation threshold are inactive
Requirements for SDS Diffusion phase:
\end{itemize}
\begin{itemize}
\item	Every agent checks a random agent’s hypothesis
\item	Agents copies hypothesis if active agent selected
\item	Agents generate random hypothesis if inactive agents selected
\end{itemize}
Requirements for Agents
\begin{itemize}
\item Agents are set to inactive by default
\item	Able to generate random hypothesis
\item	Hypotheses are minimum spanning trees
\item	Hypothesis must be accessible for external classes
\end{itemize}
I have not been able to implement the Unit tests for SDS and the SDS class itself. I have been working on the Agent classes and Unit test. I have completed the list of requirements and tests for the Agent class. However, I will need to create a method to randomly generate spanning trees in order to pass the tests.
I am still researching degree constrained minimum spanning tree papers and the benchmarks used to test them. I believe that it will be possible to modify SDS to cater for these types of spanning trees.\\
...
\item Implement Unit tests for SDS
\item	Implement SDS to pass tests
\item	Create Requirements for DFS class
\item	Implement Unit tests for DFS class
\item	Pass all Unit tests for DFS class
\item	Pass all Unit tests for Agent class
\item	Create Gantt Chart
\end{itemize}
This week’s progress
I have created the list of requirements for the DFS class that is used to check if a graph has cycles as well as if the graph is a spanning tree.
...
\item	Implement Unit tests for SDS
\item	Implement SDS to pass tests
\item	Implement quicksort for agents
\item	Create Gantt Chart
\end{itemize}
This week’s progress
The program is nearing completion which will allow for benchmarking tests to be carried out.
Unfortunately I have been unable to make any real progress this week due to a multitude of problems. The first one was how to test each step at the appropriate stage during SDS. This arose due to the design of SDS which meant that the original skeleton had to be redesigned. This then lead to the problem of running each phase method from the unit testing class as each phase method is private to incorporate encapsulation.
I am still trying to find solutions to these problems in order to create the unit test correctly and implement SDS. Until then all efforts will be focused on this issue as no progress can be made until this has been corrected.
Following the design of SDS a sorting algorithm would be required during the initialisation phase. I chose to use quicksort and have implemented a version that is specialised to sort agents.\\
